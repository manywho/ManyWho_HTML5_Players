...
(function ($) {
	...
	var generateTableDataHtml = function (domId, field, objectData, formMetaData, outcomeResponses, onClickFunction, isOffset, hasMoreResults) {
		...
		if ($(window).width() < 980) {
			...
			//TEST
			html += '<ul data-role="listview" data-inset="true" class="nav nav-tabs nav-stacked">';
			...
			if (objectData != null &&
				...
				for (var i = 0; i < objectData.length; i++) {
					...
					if (acceptedOutcomeResponses != null &&
						...
						html += '<a href="#" class="manywho-table-outcome-action" data-outcomeid="' + acceptedOutcomeResponses[0].id + '">';
					} else {
	...
	$.fn.manywhoFormBootStrap.defaults = { addRealtime: false, label: '', sectionFormat: 'tabs', columnFormat: 'accordian', toggleHtml: null, register: null, tableResultSetSize: 10, selectResultSetSize: 10 };
